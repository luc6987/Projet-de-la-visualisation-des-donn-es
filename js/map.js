...
function makeGeo(vaccinData) {
    ...
    Promise.all([
        ...
        // Color scale: Viridis interpolation from 0% to 100% uptake
        const colorScale = d3.scaleSequential(d3.interpolateViridis)
            .domain([0, 100]);

        // Draw background map (semi-transparent) - same as cartogram.js
...
function highlightCountries(country1, country2) {
    ...
    d3.selectAll(`#mapArea path[data-country="${country2}"]`)
        ...
        .style('stroke', '#e74c3c')
        .style('stroke-width', 3)
        .style('filter', 'drop-shadow(0 0 10px #e74c3c)');
...
async function addCovidOverlay(type, year) {
    ...
    try {
        ...
        // Color based on type
        const fillColor = type === 'deaths' ? '#e74c3c' : '#3498db';
