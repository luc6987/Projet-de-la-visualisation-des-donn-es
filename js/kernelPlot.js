...
function plotRidgelineKDE(data, countryCode) {
    ...
    const yScale = d3.scaleLinear()
        ...
        .range([height,0]);  // Now rows go from top to bottom normally
...
function plotTwoCountryComparison(data, countryCode1, countryCode2) {
    ...
    allVaccines.forEach((vaccine, idx) => {
        ...
        if (vaccineData1[vaccine]) {
            ...
            // Draw filled area for country 1 with animation
            svg.append('path')
                .datum(pathData1)
            ...
            // Draw outline for country 1 with animation
            svg.append('path')
                .datum(pathData1)
        ...
        if (vaccineData2[vaccine]) {
            ...
            // Draw filled area for country 2 with animation
            svg.append('path')
                .datum(pathData2)
                ...
                .style('opacity', 0)
                .transition()
                .delay(idx * 50)  // Staggered entrance
                .duration(800)
                .ease(d3.easeCubicOut)
                .style('opacity', 0.5);
            
            // Draw outline for country 2 with animation
            svg.append('path')
                .datum(pathData2)
    ...
    // Y-axis (vaccine labels) with fade-in animation
    const yAxis = d3.axisLeft(yScale)
    ...
    // Add legend with fade-in animation
