...
function getMonthlyData(data, countryCode, startYear, endYear) {
    ...
    if (countryData.length === 0) {
        ...
        console.log("Available countries:", [...new Set(data.map(d => d.ReportingCountry))]);
        return null;
    }
    
    console.log(`Found ${countryData.length} records for ${countryCode}`);
    const result = aggregateMonthlyData(countryData, startYear, endYear);
    console.log(`Aggregated to ${result ? result.length : 0} months`);
    return result;
}
...
function plotPyramidBarCharts(data, countryCode1, countryCode2, startYear = 2020, endYear = 2023) {
    ...
    if (!monthlyArray1 || !monthlyArray2 || monthlyArray1.length === 0 || monthlyArray2.length === 0) {
        console.error("Cannot create pyramid charts due to missing data");
        console.error(`Country 1 (${countryCode1}):`, monthlyArray1);
        console.error(`Country 2 (${countryCode2}):`, monthlyArray2);
        return;
    }
    
    console.log(`📊 Creating pyramid chart for ${countryCode1} vs ${countryCode2}`);
    console.log(`  ${countryCode1}: ${monthlyArray1.length} months`);
    console.log(`  ${countryCode2}: ${monthlyArray2.length} months`);
    console.log(`  Sample data 1:`, monthlyArray1[0]);
    console.log(`  Sample data 2:`, monthlyArray2[0]);
    ...
    const maxTotal = Math.max(
        d3.max(monthlyArray1, d => {
            if (!d) return 0;
            return doseTypes.reduce((sum, type) => sum + (d[type] || 0), 0);
        }) || 0,
        d3.max(monthlyArray2, d => {
            if (!d) return 0;
            return doseTypes.reduce((sum, type) => sum + (d[type] || 0), 0);
        }) || 0
    );
    
    console.log(`Max total doses: ${maxTotal}`);
...
function createHorizontalChart(svg, monthlyArray, dataMap, allMonths, xScale, yScale, xOffset, isLeft) {
    ...
    xAxisGroup.selectAll(".domain").style("stroke", "#a5b4fc");
    xAxisGroup.selectAll(".tick line").style("stroke", "#a5b4fc");
...
function addCenterAxis(svg, yScale, margin, chartWidth, centerGap, height) {
    ...
    yAxisG.selectAll(".tick line").style("stroke", "#a5b4fc");
...
function addCountryLabels(svg, countryCode1, countryCode2, margin, chartWidth, centerGap) {
    ...
    svg.append("text")
        ...
        .style("font-weight", "bold")
        .style("fill", "#ff00ff")
        .style("text-shadow", "0 0 10px rgba(255, 0, 255, 0.5)")
        .style("opacity", 0)
        .text(country2Name)
        .transition()
        .duration(600)
